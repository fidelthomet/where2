document.onreadystatechange = () => {
	...
	d3.select("#datasets").html(Object.keys(schema).map(d => {
		return '<h1>' + d + '</h1><br>' +
			"<table id='table-" + d + "'><tr><th>Property</th><th>Type</th><th>Example</th></tr>" +
			Object.keys(schema[d]).map(p => "<tr><td>" + p + "</td><td>" + schema[d][p].type + "</td><td>" + schema[d][p].example + "</td></tr>").join("") + "</table>"
	}).join("<br>") + "<br><br>")
	d3.select("#opt-dataset").html(Object.keys(schema).map(d => "<option value='" + d + "'>" + d + "</option>").join(""))

	d3.select(".opt-spatial").on("change", () => {
	...
	d3.select('#sel-query-builder').on("click", () => {
		toggleView()
		map1.invalidateSize()
		map2.invalidateSize()
	})
	...
	map1.on('draw:drawstart', () => {
		if (drawingLayer)
			map1.removeLayer(drawingLayer)
	})

	map2.on('fullscreenchange', () => {
	...
	d3.select("#send").on("click", () => {
		...
		if (d3.select("#cb-query").node().checked)
			where.query = {
		...
		if (d3.select("#cb-spatial").node().checked) {
			...
			if (where.spatial.relation === "DistWithin")
				where.spatial.distance = +d3.select("input.in-spatial").property("value")
		}
		if (d3.select("#cb-properties").node().checked)
			where.properties = d3.selectAll("#container-properties .in-properties").nodes().map(el => el.checked ? el.value : false).filter(el => el)
		if (d3.select("#cb-sort").node().checked)
			where.sort = {
				by: d3.select(".opt-properties.in-sort").property("value"),
				desc: d3.select("#cb-desc").node().checked
			}
		if (d3.select("#cb-limit").node().checked)
			where.limit = d3.select(".in-limit").property("value")
		if (d3.select("#cb-offset").node().checked)
			where.offset = d3.select(".in-offset").property("value")
...
function fillProperties() {
	d3.selectAll(".opt-properties").html(Object.keys(schema[d3.select("#opt-dataset").property("value")]).map(d => "<option value='" + d + "'>" + d + "</option>").join(""))
	d3.select("#container-properties").html(Object.keys(schema[d3.select("#opt-dataset").property("value")]).map(d => '<label><input class="in-properties" type="checkbox" checked id="cbox1" value="' + d + '">&nbsp;' + d + '</label>&nbsp;<wbr>').join(""))
}